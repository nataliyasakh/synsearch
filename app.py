...
if st.session_state.page == "search":
    ...
    if query or search_btn:
        ...
        # Answer card
        st.markdown(f"""
        <div class='ss-answer-card'>
          <div class='ss-answer-eyebrow'>Synthesised answer · {len(sources)} sources</div>
          <div class='ss-answer-text'>{answer}</div>
          <div class='ss-sources-label'>Sources — click to open original wiki</div>
          {"".join(f'''
          <a href="{s["url"]}" target="_blank" style="text-decoration:none;">
            <div class='ss-source-row'>
              <div class='ss-source-num'>[{s["num"]}]</div>
              <div style='flex:1'>
                <div class='ss-source-team'>{s["team"]}</div>
                <div class='ss-source-meta'>{s["track"]} · {s["url"].replace("https://","")}</div>
              </div>
              {medal_badge(s["medal"])}
            </div>
          </a>''' for s in sources)}
