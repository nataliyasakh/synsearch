...
DEMO_BENCH = [
    ...
    {"name": "Groq fast + SynSearch RAG", "desc": "Fast model grounded in 1,000+ iGEM wikis (2015–2025) via Pinecone vector retrieval.", "f": 0.42, "r": 0.19, "c": 0.30, "color": "#d6884a"},
...
if st.session_state.page == "search":
    st.markdown("""
    <div class='hero'>
      <div class='hero-eyebrow'>iGEM knowledge retrieval &middot; 1,000+ wikis &middot; 2015–2025 corpus</div>
    ...
    if st.button("Find similar projects", key="sgo") or sq:
        ...
        for s in real_sim:
            ...
            card = ("<div style='display:flex;flex-direction:column'>"
                    f"<a href='{card_url}' target='_blank' class='sim-card' style='flex:1'>"
...
# ABOUT
    ...
    <div class='about-card'><div class='about-h'>Building on Munich 2024</div><div class='about-p'>The Munich 2024 iGEM team built the first proof-of-concept RAG system over iGEM wiki data, demonstrating that grounding an LLM in retrieved documents reduces hallucination on synthetic biology questions. Their system ingested 343 teams from the 2019 competition and showed measurable improvement in answer quality — but left several capabilities explicitly unfinished: source citations, metadata filtering, and accuracy benchmarking were all listed as future work. The tool also required Docker and a personal API key to run, limiting adoption by non-technical users.<br><br>SynSearch builds directly on their foundation. We reuse their 2019 corpus under CC BY 4.0 and independently contributed:<ul class='about-ul'><li>Corpus expansion from 343 teams (2019 only) to 1,000+ teams across 2015–2025</li><li>Inline source citations linking to the specific wiki page — Munich's own listed future work</li><li>Metadata filtering by year, village, and medal before retrieval</li><li>A "Similar Projects" semantic explorer — not available in any existing registry</li><li>Published three-way benchmark: bare LLM vs RAG on 50 manually verified questions</li><li>Zero-install hosted interface — no Docker, no terminal, no API key required</li></ul></div></div>
    <div class='about-card'><div class='about-h'>Stack</div><div class='about-p'>sentence-transformers (all-MiniLM-L6-v2) &middot; Pinecone (70,229 vectors) &middot; Groq API &middot; Streamlit Cloud &middot; Munich 2024 corpus (CC BY 4.0)</div></div>
    <div class='about-card'><div class='about-h'>Data attribution</div><div class='about-p'>The 2019 iGEM wiki corpus was scraped and curated by the <a href='https://gitlab.igem.org/2024/software-tools/munich' target='_blank'>Munich 2024 iGEM team</a> and is used under Creative Commons Attribution 4.0. We are grateful for their work. The 2015–2025 corpus was independently scraped from igem.wiki and via the Wayback Machine using the public iGEM API.</div></div>
