...
def badge(medal):
    if not medal or medal in ("-","Unknown",""): return ""
    m = medal.lower()
...
def sbar(pct): return f"<div class='score-bar'><div class='score-fill' style='width:{pct}%'></div></div>"
def mbar(val, color): return f"<div class='mbar-bg'><div class='mbar' style='width:{int(val*100)}%;background:{color}'></div></div>"
...
if st.session_state.page == "search":
    ...
    if go or (query and query.strip()):
        ...
        for s in real_similar:
            village = s.get("track",""); med = s.get("medal","")
...
# BENCHMARK
    ...
    st.markdown("<div class='about-card' style='margin-top:12px'><div class='about-h'>Methodology</div><div class='about-p'>50 questions were written based on real queries a new iGEM team member would ask, spanning Diagnostics, Foundational Advance, Environment, and Manufacturing tracks. Correct answers and source wikis were verified manually before evaluation. Scoring uses an LLM-as-judge approach following the RAGAS framework (Es et al., 2023), measuring faithfulness, answer relevancy, and context recall.</div></div>", unsafe_allow_html=True)

# ABOUT
elif st.session_state.page == "about":
    st.markdown("<div style='padding:64px 0 32px'><div class='hero-eyebrow'>iGEM 2025 Software &amp; AI Village</div><div class='hero-title' style='font-size:48px;text-align:left'>About<br><em>SynSearch</em></div></div>", unsafe_allow_html=True)
    st.markdown("""
